= Lab 8: Introduction to Red Hat Advanced Cluster Security (RHACS)

== Learning objectives

By the end of this lab, you will be able to:

* Navigate the RHACS console and understand its main security dashboards
* Use global search and local page filtering to find security information
* Explore vulnerability management capabilities and CVE analysis
* Review compliance reporting against industry standards (PCI DSS, NIST, CIS)
* Understand network segmentation visualization and policy enforcement
* Access security policy management and violation tracking
* Explore configuration management and risk-based prioritization

== Value proposition

*For field teams*: This lab demonstrates RHACS as a comprehensive Kubernetes security platform that consolidates vulnerability scanning, compliance reporting, network visualization, and runtime threat detection into a single solution. Use this to show how RHACS eliminates the need for multiple point security tools.

*Customer proof points*:

* *Comprehensive visibility*: Single dashboard for security across development, build, and runtime stages
* *Compliance automation*: Automated reporting against PCI DSS, HIPAA, NIST, CIS Benchmarks eliminates manual audit preparation
* *Risk-based prioritization*: Contextual risk scoring helps security teams focus on the most critical vulnerabilities first
* *Shift-left security*: Identify and block vulnerabilities at build time, reducing production security incidents by enabling developers to fix issues earlier
...
====
For example, to find all violations in the *microsweeper-appservice* deployment, the search query is select Deployment *THEN* microsweeper-appservice.
====

In this search query, Deployment is the attribute, and *microsweeper-appservice* is the search term.
...
You can also search using *REGEX-like capabilities*. Add an *r/* before the search term to find all possibilities. For example, if you search for *Namespace:r/micro*, the results include matches from namespace starting with micro.
...
For example, filtering in the violations window by the microsweeper-ex namespace.
...
> *To see results, sort by Cluster -> Namespace -> Deployment. Select openshift-gitops namespace.*

image::01-network-1new.png[link=self, window=blank, width=100%, Dashboard Filter]
...
image::01-risk-1new.png[link=self, window=blank, width=100%]
...
image::01-risk-2new.png[link=self, window=blank, width=100%]
...
Excellent work! You have successfully explored the Red Hat Advanced Cluster Security (RHACS) console and its comprehensive security capabilities.

=== What you accomplished

In this lab, you:

* ✅ Navigated the RHACS console dashboard and explored security widgets
* ✅ Used global search and local filtering to find security information
* ✅ Explored vulnerability management capabilities across container images
* ✅ Reviewed compliance reporting dashboards for PCI DSS, NIST, and CIS Benchmarks
* ✅ Examined network segmentation visualization and policies
* ✅ Accessed security policy management and violation tracking
* ✅ Explored platform configuration options and system health monitoring

=== Key takeaways for customer conversations

* *Unified security platform*: Single dashboard eliminates the need for multiple security tools across development and runtime
* *Shift-left security*: Vulnerability scanning at build time prevents issues from reaching production
* *Compliance automation*: One-click audit reports against multiple compliance frameworks save weeks of manual work
* *Risk-based prioritization*: Contextual risk scoring helps teams focus on the most critical security issues first

=== Next steps

In the next lab, you will use RHACS to detect and prevent the Log4Shell vulnerability (CVE-2021-44228), demonstrating runtime policy enforcement and threat detection capabilities.
