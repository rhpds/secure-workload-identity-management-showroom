...
*For sales engineers*: This lab demonstrates ROSA's most critical security capability – eliminating static credentials through IRSA. This is the #1 security concern customers raise about Kubernetes-AWS integration. Use this to show how ROSA solves credential management better than self-managed Kubernetes.
...
* ✅ Created AWS DynamoDB table for application data (demonstrating multi-service architecture)
* ✅ Configured OIDC trust policy for secure service account authentication (showing zero trust implementation)
* ✅ Implemented IAM Roles for Service Accounts (IRSA) eliminating static credentials (proving security best practices)
* ✅ Deployed Quarkus application using Source-to-Image (S2I) build (highlighting developer productivity features)
* ✅ Explored OpenShift BuildConfigs, ImageStreams, and Deployments (understanding platform automation)
* ✅ Exposed application via OpenShift Routes for public access (demonstrating ingress management)
* ✅ Verified application functionality with AWS service integration (proving end-to-end security)
...
* *Operational efficiency*: No credential distribution infrastructure needed—eliminates Vault clusters, secret management overhead, and key distribution systems
...
1. *Pod requests AWS access* → ServiceAccount annotated with IAM role ARN
2. *ROSA injects OIDC token* → Kubernetes token signed by cluster's OIDC provider
3. *AWS STS validates token* → Verifies token signature against OIDC provider
4. *Temporary credentials issued* → 1-hour TTL credentials with scoped IAM permissions
5. *Application uses credentials* → AWS SDK automatically handles token refresh
